...
class SecondaryConfig:
    """Secondary gateway channel config (HONEST-ONLY, disabled by default)."""

    enabled: bool = False
    tier: str = "T1"
    primary_channel: str = ""
    totp_required: bool = True
    single_provider_enforce: bool = False
    t5_explicit_opt_in: bool = False
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "single_provider_enforce": self.single_provider_enforce,
            "t5_explicit_opt_in": self.t5_explicit_opt_in,
        }

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "SecondaryConfig":
        if not isinstance(data, dict):
            return cls()
        tier = str(data.get("tier", "T1") or "T1").strip().upper()
        if tier not in {"T1", "T2", "T3", "T4", "T5"}:
            tier = "T1"
        return cls(
            enabled=_coerce_bool(data.get("enabled"), False),
            tier=tier,
            primary_channel=str(data.get("primary_channel") or "").strip(),
            totp_required=_coerce_bool(data.get("totp_required"), True),
            single_provider_enforce=_coerce_bool(data.get("single_provider_enforce"), False),
            t5_explicit_opt_in=_coerce_bool(data.get("t5_explicit_opt_in"), False),
        )


@dataclass
class TotpConfig:
    """TOTP verifier settings for the secondary channel."""

    issuer: str = "hermes-secondary"
    window: int = 1

    def to_dict(self) -> Dict[str, Any]:
        return {"issuer": self.issuer, "window": self.window}

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "TotpConfig":
        if not isinstance(data, dict):
            return cls()
        try:
            ...
            window = 1
        window = max(0, min(window, 2))
        issuer = str(data.get("issuer", "hermes-secondary") or "hermes-secondary").strip()
        return cls(issuer=issuer, window=window)
...
class GatewayConfig:
    ...
    # Secondary gateway channel (HONEST-ONLY, disabled by default)
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        return cls(
            ...
            secondary=SecondaryConfig.from_dict(data.get("secondary", {})),
            totp=TotpConfig.from_dict(data.get("totp", {})),
...
def load_gateway_config() -> GatewayConfig:
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # Secondary channel + TOTP verifier (HONEST-ONLY, disabled by default).
            # Top-level ``secondary:`` / ``totp:`` win; nested ``gateway.``
            # keys are accepted for `hermes config set gateway.secondary.*`.
            _secondary_cfg = yaml_cfg.get("secondary")
            if not isinstance(_secondary_cfg, dict):
                _secondary_cfg = yaml_cfg.get("gateway", {}).get("secondary")
            if isinstance(_secondary_cfg, dict):
                gw_data["secondary"] = _secondary_cfg
            _totp_cfg = yaml_cfg.get("totp")
            if not isinstance(_totp_cfg, dict):
                _totp_cfg = yaml_cfg.get("gateway", {}).get("totp")
            if isinstance(_totp_cfg, dict):
                gw_data["totp"] = _totp_cfg
