...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # ─── Secondary gateway auth (TOTP for the secondary channel) ─────────
    # Pinned to match the `secondary-auth` extra in pyproject.toml.
    # qrcode pin (7.4.2) matches the existing messaging/dingtalk/feishu pins.
    "auth.secondary": (
        "pyotp==2.9.0",
        "qrcode==7.4.2",
    ),
