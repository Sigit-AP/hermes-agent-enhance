...
def setup_agent_settings(config: dict):
    ...
    # ── Operating Execution Profile ──
    print_header("Operating Execution Profile")
    print_info("Pilih profil operasional eksekusi Hermes Agent:")
    print_info("  1. Full Access / Headless Automation (Zero-Block, Unrestricted Tools, Tier-3 Host Protection)")
    print_info("  2. Developer Pro Mode (Auto-Approve Shell/File edits, Standard Prompts)")
    print_info("  3. Standard Balanced Mode (Interactive Confirmations for High-Risk Actions)")
    print_info("  4. Strict Sandbox Mode (Full TTY Approvals + Strict Security Scanner)")
    ...
    if p_idx == 0:  # Full Access
        ...
        config["approvals"]["cron_mode"] = "approve"
        config["approvals"]["mcp_reload_confirm"] = False
        config["approvals"]["destructive_slash_confirm"] = False
        config["hooks_auto_accept"] = True
        config.setdefault("security", {})["allow_private_urls"] = True
        config["security"]["tirith_enabled"] = False
        print_success("Execution Profile diatur ke: FULL ACCESS (Zero-Block & Tier-3 Protected)")
    elif p_idx == 1:  # Developer Pro
        config.setdefault("approvals", {})["mode"] = "smart"
        config["approvals"]["cron_mode"] = "approve"
        config["approvals"]["mcp_reload_confirm"] = False
        config["approvals"]["destructive_slash_confirm"] = True
        config["hooks_auto_accept"] = True
        config.setdefault("security", {})["allow_private_urls"] = True
        config["security"]["tirith_enabled"] = False
        print_success("Execution Profile diatur ke: DEVELOPER PRO")
    elif p_idx == 2:  # Standard
        config.setdefault("approvals", {})["mode"] = "manual"
        config["approvals"]["cron_mode"] = "deny"
        config["approvals"]["mcp_reload_confirm"] = True
        config["approvals"]["destructive_slash_confirm"] = True
        config["hooks_auto_accept"] = False
        config.setdefault("security", {})["allow_private_urls"] = False
        config["security"]["tirith_enabled"] = True
        print_success("Execution Profile diatur ke: STANDARD BALANCED")
    elif p_idx == 3:  # Strict
        config.setdefault("approvals", {})["mode"] = "manual"
        config["approvals"]["cron_mode"] = "deny"
        config["approvals"]["mcp_reload_confirm"] = True
        ...
        config.setdefault("security", {})["allow_private_urls"] = False
        config["security"]["tirith_enabled"] = True
        print_success("Execution Profile diatur ke: STRICT SANDBOX")
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        save_env_value("MATRIX_DM_POLICY", "open" if not allowed_users else "allowlist")
...
def _setup_bluebubbles():
    ...
    save_env_value("BLUEBUBBLES_DM_POLICY", "open" if not allowed_users else "allowlist")
...
def run_setup_wizard(args):
    ...
    if is_existing:
        ...
        setup_mode = prompt_choice("How would you like to set up Hermes?", [
            "Quick setup — provider, model, execution profile & messaging (recommended)",
            "Full setup — configure everything step-by-step",
        ], 0)
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    # Step 2.5: Select Operating Execution Profile
    print_header("Operating Execution Profile")
    print_info("Pilih profil eksekusi operasional:")
    print_info("  1. Full Access / Headless Automation (Zero-Block, Unrestricted Tools, Tier-3 VPS Protection)")
    print_info("  2. Developer Pro Mode (Auto-Approve Shell/File edits)")
    print_info("  3. Standard Balanced Mode (Default)")
    print_info("  4. Strict Sandbox Mode (Full Approvals)")
    ...
    if p_idx == 0:
        ...
        config.setdefault("security", {})["allow_private_urls"] = True
        config["security"]["tirith_enabled"] = False
        print_success("Execution Profile: FULL ACCESS (Zero-Block & Tier-3 Protected)")
    elif p_idx == 1:
        config.setdefault("approvals", {})["mode"] = "smart"
        config["approvals"]["cron_mode"] = "approve"
        config["approvals"]["mcp_reload_confirm"] = False
        ...
        config.setdefault("security", {})["allow_private_urls"] = True
        config["security"]["tirith_enabled"] = False
        print_success("Execution Profile: DEVELOPER PRO")
    elif p_idx == 2:
        config.setdefault("approvals", {})["mode"] = "manual"
        config["approvals"]["cron_mode"] = "deny"
        config["approvals"]["mcp_reload_confirm"] = True
        ...
        config.setdefault("security", {})["allow_private_urls"] = False
        config["security"]["tirith_enabled"] = True
        print_success("Execution Profile: STANDARD BALANCED")
    elif p_idx == 3:
        config.setdefault("approvals", {})["mode"] = "manual"
        config["approvals"]["cron_mode"] = "deny"
        config["approvals"]["mcp_reload_confirm"] = True
        ...
        config.setdefault("security", {})["allow_private_urls"] = False
        config["security"]["tirith_enabled"] = True
        print_success("Execution Profile: STRICT SANDBOX")
